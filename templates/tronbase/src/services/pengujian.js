...
export const getAllPaketPengujian = async (store, router, page=1, id) => {
  const st = store
  const rt = router
  const pg = page
  const i = id
  ...
  try {
    if (page === 1) store.commit("pengujian/resetAllPaketPengujianMutation")

    const res = await st.dispatch("pengujian/getPaketPengujianAction", {page: pg, id: i})
    store.commit("pengujian/setAllPaketPengujianMutation", {
      count: res.data["count"] ? res.data["count"] : 0,
      results: res.data["results"] ? res.data["results"] : [],
      next: res.data["next"] ? res.data["next"] : null,
      previous: res.data["previous"] ? res.data["previous"] : null
